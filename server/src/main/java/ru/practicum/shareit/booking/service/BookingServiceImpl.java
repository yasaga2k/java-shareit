...
public class BookingServiceImpl implements BookingService {
    ...
    public List<BookingResponseDto> getAllByUser(Long userId, String state) {
        ...
        List<Booking> bookings = switch (state.toUpperCase()) {
            ...
            case "CURRENT" ->
                    bookingRepository.findByBookerAndStartIsBeforeAndEndIsAfterOrderByStartDesc(user, LocalDateTime.now(), LocalDateTime.now());
    ...
    public List<BookingResponseDto> getAllByOwner(Long ownerId, String state) {
        ...
        List<Booking> bookings = switch (state.toUpperCase()) {
            ...
            case "CURRENT" ->
                    bookingRepository.findByItemOwnerAndStartIsBeforeAndEndIsAfterOrderByStartDesc(owner, LocalDateTime.now(), LocalDateTime.now());
            case "PAST" -> bookingRepository.findByItemOwnerAndEndIsBeforeOrderByStartDesc(owner, LocalDateTime.now());
            case "FUTURE" ->
                    bookingRepository.findByItemOwnerAndStartIsAfterOrderByStartDesc(owner, LocalDateTime.now());
            case "WAITING" -> bookingRepository.findByItemOwnerAndStatusOrderByStartDesc(owner, BookingStatus.WAITING);
            case "REJECTED" ->
                    bookingRepository.findByItemOwnerAndStatusOrderByStartDesc(owner, BookingStatus.REJECTED);
            default -> throw new IllegalArgumentException("Неизвестный статус: " + state);
        };
